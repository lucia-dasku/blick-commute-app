...
export async function runLiveActivityPublicationPolicyMigration(
  ...
  if (!connectionString) {
    throw new Error(
      "LIVE_COMMUTE_MIGRATION_DATABASE_URL is required to run live activity publication policy migrations",
    );
  }
...
if (
  ...
  await runLiveActivityPublicationPolicyMigration();
}
