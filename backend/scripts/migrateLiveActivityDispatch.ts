...
export async function runLiveActivityDispatchMigration(
  ...
  if (!connectionString) {
    throw new Error(
      "LIVE_COMMUTE_MIGRATION_DATABASE_URL is required to run live activity dispatch migrations",
    );
  }
...
if (
  ...
  await runLiveActivityDispatchMigration();
}
