...
export async function runLiveActivityPublicationCyclesMigration(
  ...
  if (!connectionString) {
    throw new Error(
      "LIVE_COMMUTE_MIGRATION_DATABASE_URL is required to run live activity publication cycle migrations",
    );
  }
  ...
  const migrationPath = fileURLToPath(
    new URL("../migrations/006_live_activity_publication_cycles.sql", import.meta.url),
  );
...
if (
  ...
  await runLiveActivityPublicationCyclesMigration();
}
