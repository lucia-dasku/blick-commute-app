...
export async function runLiveActivityClientPublicationStateMigration(
  ...
  if (!connectionString) {
    throw new Error(
      "LIVE_COMMUTE_MIGRATION_DATABASE_URL is required to run live activity client state migrations",
    );
  }
...
if (
  ...
  await runLiveActivityClientPublicationStateMigration();
}
