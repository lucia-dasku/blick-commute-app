...
class StopRoutineNotificationActionTest {
    ...
    @Test
    fun `stop pauses the routine for today in the device's own zone, reschedules, and removes the notification`() = runTest {
        val repository = FakeRoutineRepository(routine())
        val scheduler = RecordingScheduler()
        val notifier = RecordingNotifier()

        StopRoutineNotificationAction(repository, scheduler, notifier, clock, stockholmZone).stop("r1")
    ...
    @Test
    fun `stop removes the notification even when the routine has already been deleted`() = runTest {
        val repository = FakeRoutineRepository(routine = null)
        val scheduler = RecordingScheduler()
        val notifier = RecordingNotifier()

        StopRoutineNotificationAction(repository, scheduler, notifier, clock, stockholmZone).stop("gone")
